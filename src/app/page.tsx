...
export default async function HomePage() {
  ...
  try {
    ...
    // manifest unavailable — show empty state
  }
  ...
  return (
    <div className="min-h-screen bg-zinc-50 font-sans">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-10">
        {catalogs.length > 0 && (
          <Section title="产品图册" items={catalogs} hrefPrefix="/c" />
        )}
        {distributions.length > 0 && (
          <Section title="分销专供图册" items={distributions} hrefPrefix="/distributions" />
        )}
        {catalogs.length === 0 && distributions.length === 0 && (
          <div className="text-center py-20 text-zinc-400">
            <p className="text-lg font-semibold">暂无可用的图册</p>
            <p className="text-sm mt-2">请确认 catalog-manifest.json 已发布至 R2</p>
          </div>
        )}
      </main>
...
function Section({ title, items, hrefPrefix }: {
  title: string
  items: (CatalogManifestItem | DistributionManifestItem)[]
  hrefPrefix: string
}) {
  return (
    <section>
